(function() {
  var EE, PrettyError, Promise, _, appData, baseEmitter, clientSideError, configs, cwd, errorMessage, fileProcessors, getOutputPath, log, path, pe, plugins, savedState, setDefaultPreprocessor;
  ...
  module.exports = {
    ...
    getFile: function(filePath, config, options) {
      var fileProcessor, preprocessor, preprocessorConfig;
      if (options == null) {
        options = {};
      }
      filePath = path.join(config.projectRoot, filePath);
      log("getFile " + filePath);
      if (!(preprocessorConfig = configs[filePath])) {
        ...
        preprocessorConfig.on("rerun", function() {
          return baseEmitter.emit("file:updated", filePath);
        });
        baseEmitter.once("close", function() {
          return preprocessorConfig.emit("close");
        });
      }
      ...
      preprocessor = fileProcessors[filePath] = Promise["try"](function() {
        return plugins.execute("file:preprocessor", preprocessorConfig);
      });
      return preprocessor;
    },
