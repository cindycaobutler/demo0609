(function() {
  var DEFAULT_DOMAIN_NAME, Promise, Request, Server, Socket, _, allowDestroy, appData, buffers, check, connect, cookie, cors, cwd, errors, evilDns, express, fileServer, fullyQualifiedRe, hbs, headersUtil, http, httpProxy, httpsProxy, la, log, logger, origin, setProxiedUrl, statusCode, stream, url;
  ...
  Server = (function() {
    ...
    Server.prototype.createServer = function(app, config, request) {
      return new Promise((function(_this) {
        return function(resolve, reject) {
          var baseUrl, callListeners, fileServerFolder, onError, onSniUpgrade, onUpgrade, port, socketIoRoute;
          port = config.port, fileServerFolder = config.fileServerFolder, socketIoRoute = config.socketIoRoute, baseUrl = config.baseUrl;
          ...
          _this._server.on("connect", function(req, socket, head) {
            ...
            return _this._httpsProxy.connect(req, socket, head, {
              onDirectConnection: function(req) {
                ...
                log("HTTPS request " + word + " match URL: " + urlToCheck + " with props: %o", _this._remoteProps);
                return !isMatching;
              }
            });
          });
    ...
    Server.prototype._onResolveUrl = function(urlStr, headers, automationRequest) {
            ...
            handleReqStream = function(str) {
              ...
              return pt = str.on("error", error).on("response", function(incomingRes) {
                ...
                return request.setJarCookies(jar, automationRequest).then(function(c) {
                  var contentType, details, fp, isHtml, isOk;
    ...
    Server.prototype._onDomainSet = function(fullyQualifiedUrl) {
      var l;
      l = function(type, url) {
        return log("Setting %s %s", type, url);
      };
      if (fullyQualifiedUrl === "<root>" || !fullyQualifiedRe.test(fullyQualifiedUrl)) {
        ...
        this._remoteFileServer = "http://" + DEFAULT_DOMAIN_NAME + ":" + (this._fileServer.port());
