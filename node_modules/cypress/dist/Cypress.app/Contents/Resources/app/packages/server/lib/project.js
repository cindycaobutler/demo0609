(function() {
  var Automation, EE, Project, Promise, R, Reporter, Server, Watchers, _, api, browsers, cache, check, commitInfo, config, cwd, errors, files, fs, glob, ids, la, localCwd, log, logger, multipleForwardSlashesRe, path, plugins, preprocessor, savedState, scaffold, scaffoldLog, settings, user,
  ...
  Project = (function(superClass) {
    ...
    function Project(projectRoot) {
      ...
      log("Project created %s", this.projectRoot);
    }
    ...
    Project.prototype.open = function(options) {
      ...
      log("opening project instance %s", this.projectRoot);
      this.server = Server();
      _.defaults(options, {
        report: false,
      ...
      return this.getConfig(options).then((function(_this) {
        return function(cfg) {
          ...
          return _this.server.open(cfg, _this).spread(function(port, warning) {
            ...
            return Promise.join(_this.watchSettingsAndStartWebsockets(options, cfg), _this.scaffold(cfg)).then(function() {
              return Promise.join(_this.watchSupportFile(cfg), _this.watchPluginsFile(cfg, options));
            }).then(function() {
              return _this._initPlugins(cfg, options);
            });
          });
        };
      })(this))["return"](this);
    ...
    Project.prototype._initPlugins = function(config, options) {
    ...
    Project.prototype.reset = function() {
      log("resetting project instance %s", this.projectRoot);
    ...
    Project.prototype.close = function() {
      var ref, ref1;
      log("closing project instance %s", this.projectRoot);
      if (this.memoryCheck) {
    ...
    Project.prototype.watchSupportFile = function(config) {
      var supportFile;
      if (supportFile = config.supportFile) {
        return fs.pathExists(supportFile).then((function(_this) {
          return function(found) {
            var options, relativePath;
            if (!found) {
              errors["throw"]("SUPPORT_FILE_NOT_FOUND", supportFile);
            }
            relativePath = path.relative(config.projectRoot, config.supportFile);
            if (config.watchForFileChanges !== false) {
              options = {
                onChange: _.bind(_this.server.onTestFileChange, _this.server, relativePath)
              };
            }
            return preprocessor.getFile(relativePath, config, options)["catch"](function() {});
          };
        })(this));
      } else {
    ...
    Project.prototype.watchPluginsFile = function(config, options) {
      log("attempt watch plugins file: " + config.pluginsFile);
      if (!config.pluginsFile) {
        return Promise.resolve();
      }
      return fs.pathExists(config.pluginsFile).then((function(_this) {
        return function(found) {
          ...
          return _this.watchers.watch(config.pluginsFile, {
            onChange: function() {
              log("plugins file changed");
              return _this._initPlugins(config, options)["catch"](function(err) {
                return options.onError(err);
              });
            }
          });
    ...
    Project.prototype.watchSettingsAndStartWebsockets = function(options, config) {
      var paths, reporter;
      if (options == null) {
        options = {};
      }
      ...
      if (config.report) {
        if (!Reporter.isValidReporterName(config.reporter, config.projectRoot)) {
      ...
      this.automation = Automation.create(config.namespace, config.socketIoCookie, config.screenshotsFolder);
      return this.server.startWebsockets(this.automation, config, {
        ...
        onMocha: (function(_this) {
          return function(event, runnable) {
            ...
            if (event === "end") {
              stats = reporter != null ? reporter.stats() : void 0;
              _this.server.end();
    ...
    Project.prototype.scaffold = function(config) {
      var push, scaffolds;
      log("scaffolding project %s", this.projectRoot);
      scaffolds = [];
